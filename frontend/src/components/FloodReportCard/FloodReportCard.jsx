 ...
 * Props: report {village, subdivision, description, severity, status, created_at, image_url}, onPress
 */
const FloodReportCard = ({ report, onPress }) => (
  <TouchableOpacity style={cardStyles.floodCard} activeOpacity={0.85} onPress={onPress}>
    <View style={cardStyles.floodRow}>
      {report.image_url ? (
        <Image source={{ uri: report.image_url }} style={cardStyles.floodImage} />
      ) : null}

      <View style={cardStyles.floodBody}>
